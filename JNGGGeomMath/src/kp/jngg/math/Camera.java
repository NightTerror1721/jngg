...
public class Camera
{
    ...
    public final void setLimitedScope(Vector2 p0, Vector2 p1)
    {
        ...
        /*if(!display.isFullscreenEnabled())
        {
    ...
    //public final boolean contains(Focusable focusable) { return focusable.isInClip(this); }
    ...
    public final void lookAt(Vector2 gpos)
    {
        modif = true;
        pos.x = gpos.x;
        pos.y = gpos.y;
    ...
    public final void drawCross(Graphics2D g, Display display, Color color)
    {
        int halfX = display.getWidth() / 2;
        int halfY = display.getHeight() / 2;
        g.setColor(color);
        g.drawLine(0, halfY, display.getWidth(), halfY);
        g.drawLine(halfX, 0, halfX, display.getHeight());
    }
}
